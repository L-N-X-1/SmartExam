#Façade : lance le Coordinator → récupère toutes les questions → passe au Validator → retourne seulement les questions validées.# ui/pages/generate.py
"""
        ...
        with st.spinner("🤖 Les agents Bloom travaillent..."):
            try:
                ...
                for i, q in enumerate(questions[:5], 1):
                    with st.expander(f"Question {i} - Niveau {q['level_name']}"):
                        st.write(f"**{q['text']}**")
                        st.write(f"Type: {q['type']} | Points: {q['marks']} | Difficulté: {q['difficulty']}")
