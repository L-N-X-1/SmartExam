...
IMPORTANT : Réponds UNIQUEMENT avec le JSON, sans texte avant ou après, sans markdown."""
    ...
    def parse_llm_response(self, response_text):
        """
        Parse la réponse du LLM et extrait les questions (robuste)
        """
        # Nettoie la réponse : supprime Markdown
        clean_response = response_text.strip()
        clean_response = re.sub(r'```json\s*', '', clean_response)
        clean_response = re.sub(r'```\s*', '', clean_response)

        # 🔒 Supprime les caractères de contrôle invalides
        clean_response = re.sub(r"[\x00-\x09\x0b-\x1f\x7f]", "", clean_response)

        try:
            questions = json.loads(clean_response)

            # Ajoute le niveau Bloom et level_name à chaque question
            for q in questions:
                q['bloom_level'] = self.bloom_level
                q['level_name'] = self.level_name

            return questions

        except json.JSONDecodeError:
            # fallback : crée une question brute si JSON invalide
            print(f"❌ JSON invalide reçu, retour brut :\n{response_text[:500]}")
            return [{
                "text": response_text.strip(),
                "type": "open",
                "marks": 0,
                "difficulty": "medium",
                "bloom_verb": self.bloom_verbs[0] if self.bloom_verbs else "Evaluate",
